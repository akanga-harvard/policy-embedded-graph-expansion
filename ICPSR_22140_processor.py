...
class ICPSR22140Processor:
    ...
    def _load_from_checkpoint(self, checkpoint_filename: str) -> tuple[dict, int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        # We load split-level files from epoch 600
        if checkpoint_filename[-7:-4] == 'all':
            epoch_idx = 0
        else:
            epoch_idx = 40
        log_pl, step_idx, theta_unary, theta_pairwise, m_unary, v_unary, m_pairwise, v_pairwise = sorted([key + value for key, value in checkpoint_dict.items()], reverse=True)[epoch_idx]
    ...
    def set_dataset_info(self, covariates, statuses, G):
        self.theta_train_covariates = copy.deepcopy(covariates)
        self.theta_train_statuses = copy.deepcopy(statuses)
        self.theta_train_G = copy.deepcopy(G)

    def get_dataset_for_fitting_theta(self, dataset_key: str) -> tuple[dict, dict, nx.Graph, nx.DiGraph, np.ndarray, np.ndarray]:
        return self.theta_train_covariates, self.theta_train_statuses, self.theta_train_G, None, None, None

    def get_theta_parameters(self, dataset_key: str) -> tuple[np.ndarray, np.ndarray]:
        checkpoint_filename = f"{dataset_key}.pkl"
    ...
    """
    def fit_theta_parameters(self, filename: str) -> None:
        checkpoint_filename = f"checkpoints/gittins_thetas/{filename}.pkl"
        os.makedirs(os.path.dirname(checkpoint_filename), exist_ok=True)

        covariates, statuses, G, _, _, _ = self.get_dataset_for_fitting_theta(filename)
        n = G.number_of_nodes()
        ...
        if start_iter <= max_iter:
            for it in tqdm(range(start_iter, max_iter+1), desc=f"{checkpoint_filename}", leave=False):
                self.memo_local_log_ZProb = dict()
                grad_unary, grad_pairwise = self._compute_gradients(filename, theta_unary, theta_pairwise)

                # Adam update for theta_unary
                m_unary = beta1 * m_unary + (1 - beta1) * grad_unary
                v_unary = beta2 * v_unary + (1 - beta2) * (grad_unary ** 2)
                m_hat_unary = m_unary / (1 - beta1 ** it)
                v_hat_unary = v_unary / (1 - beta2 ** it)
                ...
                m_hat_pairwise = m_pairwise / (1 - beta1 ** it)
                v_hat_pairwise = v_pairwise / (1 - beta2 ** it)
                ...
                if ((it > 0 and it % 10 == 0) or it == max_iter):
                    self.memo_local_log_ZProb = dict()
                    log_pl = self._compute_log_pseudo_likelihood(filename, theta_unary, theta_pairwise)
